...
// 渲染已有的标注信息
export const renderBoxMap = () => {
  const { project } = store.getState()
  const { boundingBoxMap, currentImgInfo, currentCanvas, entityColorMap } = project

  if (boundingBoxMap.length === 0 || !currentCanvas) return
  let fBoxes = []
  const scaleFactor = 1
  if (Object.keys(boundingBoxMap).length != 0)
            ...
            return new fabric.Path(
              box.points.map(point => {
                if (point[0] === 'M' || point[0] === 'L') {
                  return [point[0], point[1] / scaleFactor, point[2] / scaleFactor]
                }
                if (point[0] === 'Q')
                  return [
                    'Q',
  ...
  fabric.util.enlivenObjects(_actualBoxMap, function (objects) {
    ...
    objects.forEach(function (o) {
      ...
      if (o.shape == 'yoloRect') {
        const text = new fabric.Text(o.label[0], {
          fontSize: 16,
          fill: 'white',
          textBackgroundColor: o.stroke,
          left: o.left,
          top: o.top - 18,
        })
        const _label = [o.label[0]]
        o = new fabric.Group([o, text])
        o.label = _label
      }
      currentCanvas.add(o)
    })
